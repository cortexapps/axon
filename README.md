...
1. Go to your Cortex [GitHub settings page](https://app.getcortexapp.com/admin/settings/github) 2. Choose "Add Configuration", then "Relay".  Choose your alias name, for example `github-relay` or any unused name.
3. Click "Save"


### Running the agent locally for testing

Next, you'll need to run the Axon agent with the following command, create a file called `.env` with the following contents, assuming you are using the public Github API:
...
If you are using a Github Enterprise installation, you'll need to set the following, your `GITHUB_API` will be like `github.mycompany.com/api/v3`, and `GITHUB_GRAPHQL` will be like `github.mycompany.com/api/graphql`.
...
Now run `docker-compose up` and you should see the agent start up and connect to Cortex.
...
```
docker run -v "/path/for/your/project:/src" ghcr.io/cortexapps/cortex-axon-agent:latest init --language python my-cortex-app 
```
...
By default, the app doesn't do anything but you can test running it:
...
# start the Axon agent (do this in a separate prompt, leave it running)
docker run -e "DRYRUN=true" -p "50051:50051" -p "80:80" ghcr.io/cortexapps/cortex-axon-agent:latest serve
...
2. You can call the Cortex API directly at any time at the address `http://localhost/cortex-api`, e.g. `GET http://localhost/cortex-api/api/v1/catalog/entities`. The Agent will automatically add your `CORTEX_API_TOKEN` to the headers of the request, and handle things like rate limitng.

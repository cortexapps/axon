...
// Add registers a new pending request using the default timeout and returns a
// channel to await the response.
func (pr *PendingRequests) Add(requestID, streamID string) <-chan *assembledResponse {
	return pr.AddWithTimeout(requestID, streamID, pr.timeout)
}

// AddWithTimeout registers a new pending request with a per-request timeout.
// If timeout <= 0, the registry's default timeout is used.
func (pr *PendingRequests) AddWithTimeout(requestID, streamID string, timeout time.Duration) <-chan *assembledResponse {
	if timeout <= 0 {
		timeout = pr.timeout
	}
...
func assembleChunks(chunks []*pb.HttpResponse) *assembledResponse {
	...
	// First chunk has status code, headers, and the failed-dispatch flag.
	first := chunks[0]
	resp := &assembledResponse{
		StatusCode:       int(first.StatusCode),
		Headers:          first.Headers,
		IsFailedDispatch: first.IsFailedDispatch,
	}

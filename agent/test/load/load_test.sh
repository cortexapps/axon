...
# Outbound tunnel connections per agent — the only tunnel dial there is.
export CONNS=${CONNS:-4}
# Settling time after chaos stops, before recovery is judged.
QUIESCE=${QUIESCE:-90}
# Tag report/log filenames so repeat runs don't overwrite each other.
...
echo "=== Agent: $CONNS outbound connections, streams on demand (tag: $RUN_TAG) ==="
    ...
    while true; do
        {
            ...
            echo
            # one agent's stream view (streams/busy/target)
...
# Long enough for the slowest chaos victim to finish restarting, reconnect
# and re-register before recovery is judged. Reconnect backoff caps at 30s,
# so anything under ~45s risks failing a run that was going to recover.
echo "=== Quiescing ${QUIESCE}s, then validating steady-state recovery ==="
sleep "$QUIESCE"
...
echo "=== Stream samples in samples.log ==="

...
#
# The models are sized to the SAME concurrent stream count, so the
# comparison isolates connection count rather than concurrency.
#
# Knobs (env): SERVERS AGENTS_PER_TOKEN LOADGENS WORKERS DURATION
#              CHAOS_INTERVAL STREAMS MODELS
...
echo "streams:  $STREAMS concurrent per agent in every model"
echo "models:   $MODELS"
echo

...
// callTable tracks the in-flight calls on one stream. In the slot-pool
// model the server sends at most one call at a time per stream, but the
// table keeps the demux honest (and cheap) if that ever changes.

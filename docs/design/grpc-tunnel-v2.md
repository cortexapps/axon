...
- Per-call flow control (send credits) would add a `CallCredit` message.
  Only needed if we abandon one-call-per-stream (§5); v1 does not.

## 5. Concurrency: on-demand streams over fixed connections

The agent holds `AXON_GRPC_TUNNEL_CONNS` connections (default 4) and opens
`Tunnel` streams over them on demand, each carrying **at most one in-flight
call at a time**. There is no concurrency setting: streams are opened as
calls arrive and given back as they finish.

**Connections are a blast-radius decision, not a load one.** Each is
load-balanced round-robin across every server instance DNS resolves to, so a
single connection already reaches the whole fleet and losing one costs
1/`CONNS` of capacity for one redial rather than all of it. Because the count
is fixed, that protection does not quietly shrink during quiet periods — the
mistake an adaptive pool makes, since incidents do not wait for peak traffic.

**Streams are what follows load.** The sizing rule is one line: hold enough
streams for the calls in flight, plus a small idle reserve, bounded by a
ceiling. Growth and shrink both fall out of it, so there is no watermark,
growth step or cooldown to tune. Resizing runs at call admission — the
instant idle capacity drops — so a burst is met by opening streams rather
than by queueing.

**The idle reserve is per server instance.** A server dispatches only onto
the streams registered with itself, so it makes callers wait as soon as its
own share of the reserve is taken, however idle the agent is overall. A flat
reserve spread over S servers leaves each with reserve/S, thinning toward
zero as the fleet grows, with latency as the only symptom. The agent
therefore scales the reserve by the number of instances it currently holds
streams on.

**Why one call per stream.** It is what makes a busy stream genuinely
unavailable, which is what gives dispatch its backpressure (below). It also
keeps gRPC's per-RPC HTTP/2 window doing the flow control for us — no
app-layer credits — and keeps a slow 100 MiB response from wedging another
call. Multiplexing many calls per stream would return both problems: the
recv loop delivers body bytes synchronously, so one slow upstream would
head-of-line block every call sharing the stream.

**Backpressure is a consequence, not a mechanism.** When the agent falls
behind, its idle streams run out; the server's `acquireStream` finds none and
waits, bounded by the caller's deadline; the delay reaches the originator as
latency. Nothing negotiates and nothing is configured. Two limits keep it
honest:

- **Stream ceiling.** Without one the agent would accept work it cannot do
  and the queue would move into its heap, where nothing can see it. A
  `CallStart` is a few hundred bytes, so h2 flow control would not push back
  for a very long time.
- **`AXON_UPSTREAM_MAX_CONNS_PER_HOST`** (default 128). The upstream is the
  customer's own service, whose capacity we neither control nor can discover.
  This also raises net/http's default of 2 idle connections per host, which
  would otherwise make every concurrent call past the second pay a fresh
  TCP+TLS handshake.
...
- **Server cap (defensive only):** the server enforces
  ...
  `ResourceExhausted`; the client clamps its ceiling accordingly. This is
  protection against a buggy agent, not a control channel.

**Observability.** `dispatch.acquire_wait_ms` and `dispatch.all_busy` record
time spent waiting for an idle stream. This is the number that separates "the
tunnel is saturated" from "the upstream got slow" — `dispatch.duration_ms`
measures the whole handler and cannot tell them apart.

Stream lifecycle mirrors PR #85's `manageStream` loop: connect → handshake
...
- `TunnelService` (`server/tunnel/service.go`) — owns bidi streams, hello
  ...
  chunk-assembler.
- `ClientRegistry` (`server/tunnel/client_registry.go`) — hands out idle
  streams via `AcquireIdleStream`, popping a LIFO idle stack so the cost
  does not grow with an agent's stream count. LIFO also prefers the most
  recently released stream, which is the one most recently proven healthy.
  Returning `(nil, allBusy=true)` is the backpressure signal the dispatcher
  turns into a bounded wait.
...
1. **Concurrency ceiling.** ~~32 concurrent calls per agent is fine for
   today's workloads but not proven for future ones.~~ Resolved: streams
   are opened on demand (§5), so concurrency is whatever the agent can
   serve. The remaining ceilings are the stream cap and the per-token
   server cap, and hitting either produces caller-visible latency rather
   than a hidden queue. Measured at 281 req/s per agent against 121-232
   for the fixed-size alternatives, with a 9x better p99.
